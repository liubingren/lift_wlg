...
var constructImgList = function constructImgList(data) {
  ...
  if (data.indexOf(',') > -1) {
    ...
    for (var i = 0; i < datas.length; i++) {
      var imgSrc = HEADER + 'workMaintain/check_getFile.do?fileUrl=' + datas[i];
    ...
    var _imgSrc = HEADER + 'workMaintain/check_getFile.do?fileUrl=' + data;
...
var init = function init() {
  if (workOrderId) {
    $.ajax({
      ...
      url: HEADER + 'workMaintain/check_getWorkMaintainByWorkOrder.do',
      success: function success(data) {
        ...
        $("#nr_idSpan").html(response.unit_id);
        $("#id_nr_input").val(response.id_nr);
        $("#maint_comp").val(response.maint_comp);
        var maintain_type_value = void 0;
        switch (response.maintain_type) {
          case "维保自检":
            maintain_type_value = '01';
            break;
          case "保障":
            maintain_type_value = '02';
            break;
          case "其他":
            maintain_type_value = '03';
            break;
        }
        $("#typeSpan").attr("spanAttr", maintain_type_value).html(response.maintain_type);
        $("#faultTime").val(response.fault_time);
        $("#phone").val(response.report_tel);
        ...
        $("#maintStaffsSpan").attr("spanAttr", response.maint_staff).html(response.maint_staff_value);
        ...
        switch (action) {
          case "watch":
            title = '\u67E5\u770B';
            addCoverInDisabled(['communitySpan', 'nr_idSpan', 'faultTime', 'typeSpan', 'phone', 'solutionSpan', 'solution_input', 'remark', 'other', 'stateSpan', 'maintStaffsSpan', 'checkbox', 'checkSpan', 'checkText']);
            $(".btn-toolbar button").click(function () {
              window.location.href = '../html/workorderManage.html?orderType=workMaintain';
            });
            var _str = '.group4_item3 .inCheckList,.group4_item3 .checkList,.group4_item4 div,.group4_item4 textarea';
            $('' + _str).css("display", "block");
            $('.group4_item4').css("height", "70px");
          ...
          case "edit":
            title = '\u4FEE\u6539';
            addCoverInDisabled(['communitySpan', 'nr_idSpan']);
            break;
          case "check":
            title = '\u5BA1\u6838';
            addCoverInDisabled(['communitySpan', 'nr_idSpan', 'faultTime', 'typeSpan', 'phone', 'solutionSpan', 'solution_input', 'remark', 'other', 'stateSpan', 'maintStaffsSpan', 'checkbox']);
            var htmlStr = '.group4_item3 .inCheckList,.group4_item3 .checkList,.group4_item4 div,.group4_item4 textarea';
...
$(".btn-toolbar button:last-child").click(function () {
  if (action === "check") {
    ...
    $.ajax({
      type: 'post',
      url: HEADER + 'workMaintain/approval_WorkMaintain.do',
      data: {
        id: id,
    ...
    var lift_id = $("#nr_idSpan").attr("spanAttr"); //32位,电梯id
    var maintain_type = $("#typeSpan").attr("spanAttr"); //2位，故障类别
    var fault_time = $("#faultTime").val(); //时间，故障时间
    var report_tel = $("#phone").val(); //报修电话
    ...
    if ($("#communitySpan").text() == "请选择小区" || $("#nr_idSpan").text() == "请选择设备内部编号" || !$("#faultTime").val() || $("#typeSpan").text() == "请选择类别" || $("#stateSpan").text() == "请选择工单状态" || $("#maintStaffsSpan").text() == "请选择维保人员") {
      secondPop(2, "请完善必填信息方能提交");
      return false;
    }
    ...
    jsonData.append('maintain_type', maintain_type);
    jsonData.append('fault_time', fault_time);
    ...
    jsonData.append('report_tel', report_tel);

    if (action === 'edit') {
      jsonData.append('id', workOrderId);
      url = "workMaintain/update_WorkMaintain.do";
    } else {
      jsonData.append('lift_id', lift_id.trim());
      url = "workMaintain/add_WorkMaintain.do";
    }
...
$(".imgList .delete").click(function () {
  ...
  if (srcArr.length !== 0) {
    ...
    $.ajax({
      ...
      url: HEADER + 'workMaintain/delete_File.do',
      beforeSend: function beforeSend() {
        loading();
      },
      success: function success(data) {
...
//# sourceMappingURL=maintain_workorderManage.js.map