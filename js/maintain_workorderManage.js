...
const constructImgList = (data) => {
  ...
  if (data.indexOf(',') > -1) {
    ...
    for (let i = 0; i < datas.length; i++) {
      let imgSrc = HEADER + 'workMaintain/check_getFile.do?fileUrl=' + datas[i];
    ...
    let imgSrc = HEADER + 'workMaintain/check_getFile.do?fileUrl=' + data;
      ...
      success: (data) => {
        ...
        switch (action) {
          case "watch":
            ...
            let str = `.group4_item3 .inCheckList,.group4_item3 .checkList,.group4_item4 div,.group4_item4 textarea`;
          ...
          case "check":
            ...
            let htmlStr = `.group4_item3 .inCheckList,.group4_item3 .checkList,.group4_item4 div,.group4_item4 textarea`;
...
$(".btn-toolbar button:last-child").click(function () {
  if (action === "check") {
    ...
    let lift_id = $("#nr_idSpan").attr("spanAttr");         //32位,电梯id
    let maintain_type = $("#typeSpan").attr("spanAttr");         //2位，故障类别
    let fault_time = $("#faultTime").val();                        //时间，故障时间
    let report_tel = $("#phone").val();      //报修电话
