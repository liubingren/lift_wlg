//修改,查看或者审核工单
const operatingOrder = (operating,dom) => {
  if(operating&&operating){
    let pa=$(dom).parent();
    let plan_id= pa.attr('plan_id');
    let workOrder= pa.attr('workOrder');
    if(plan_id){
...
const search =(pagesize,page)=>{
  ...
  let maintenanceType = $('#maintenanceSpan').attr("spanAttr");
