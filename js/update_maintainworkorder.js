...
//获取保养工单信息
const getMaintInfor =()=>{
  let url = HEADER + '/workMaintenance/check_getworkMaintenanceById.do'
  let html = ''
  let html1 =''
  $.ajax({
    type: 'get',
    ...
    success: (data) => {
      ...
      //维保项目
      let maintList = dataArr.maintItems
      let normal =''
      let Abnormal = ''
      let maintresult,maintwritten,maintText
      for(let i = 0; i < maintList.length; i++){
        maintresult = maintList[i].result
        maintwritten =  maintList[i].written
        if(maintwritten === 1){
          maintText = `<input type="text" title="${!!maintresult ? maintresult : ''}" class="maintwrite" placeholder="请输入内容" value="${!!maintresult ? maintresult : ''}" />`
      ...
      //审核
      let code1 = dataArr.check_status
      let status1
      if (code1 === 3) {
        status1 = "已审核"
      } else if (code1 === 2) {
...
const constructImgList = (data) => {
  ...
  if (data.indexOf(',') > -1) {
    ...
    for (let i = 0; i < datas.length; i++) {
      let imgSrc = HEADER + '/workMaintenance/check_getFile.do?fileUrl=' + datas[i];
    ...
    let imgSrc = HEADER + '/workMaintenance/check_getFile.do?fileUrl=' + data;
...
//获取解决方法
const solvemethod =()=> {
  let url = HEADER + '/workOrder/check_listSolution.do'
  let html = ''
  $.ajax({
    type: 'get',
    url: url,
...
//提交保养信息
const postMaintInfor =()=>{
  let url = HEADER + '/workMaintenance/update_WorkMaintenance.do'
  let jsonData = new FormData();
  let exist_problem = $("#question").val()
  //解决方法
  let solution = $("#solveMethod").html()
  let add_solveMethod = $("#add_solveMethod").val()
  let sm = ''
  if(add_solveMethod === ''){
    sm = solution
  }else {
  ...
  let flag = true
  let item_id = ''
  let trItem = $("#maintInfo tr")
  let resultCode = ''
  let  maintwritten = ''
  for(let i = 0; i < trItem.length; i++){
    item_id = $(trItem[i]).attr('data-maintitemid')
    resultCode = $(trItem[i]).attr('data-maintcode')
    maintwritten = $(trItem[i]).attr('data-wirte')
    if(maintwritten === '1'){
      resultCode = $(trItem[i]).find('input').val()
    }
    if(resultCode === 'null'){
...
//提交审核工单
const postExamineworkorder = () =>{
  let url = HEADER + '/workMaintenance/approval_WorkMaintenance.do'
