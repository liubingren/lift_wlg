...
const submitAddLift = () => {
  let url;
  let pa = $("#add-update-lift");
  let username = pa.find(".username").val();
  let password = pa.find(".password").val();
  let name = pa.find(".name").val();
  let phonenumber = pa.find(".phonenumber").val();
  let email = pa.find(".email").val();
  let sex = pa.find("#sex").text();
  let rid = pa.find("#rid").attr("spanAttr");
  let location_id = pa.find("#location_id").attr("spanAttr");
  let mc_id = pa.find("#mc_id").attr("spanAttr");
  let status;
  pa.find(".radio-wrap-group span").each(function () {
